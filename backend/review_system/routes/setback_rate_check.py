...
class SetbackRateCheckRequest(BaseModel):
    ...
    sample_step: Optional[float] = 1.0
...
def setback_rate_check(request: SetbackRateCheckRequest) -> Dict[str, Any]:
    ...
    try:
        return check_setback_rate_pure_python(
            ...
            sample_step=request.sample_step or 1.0,
            tolerance=request.tolerance or 0.5,
            required_rate=request.required_rate,
        )
    except ValueError as exc:
        logger.warning(
            "Setback rate check failed: %s (building_layer=%s, setback_layer=%s)",
            exc,
