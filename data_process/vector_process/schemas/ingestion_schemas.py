...
class DocumentIngestionState(BaseModel):
    ...
    processed_chunks: int = Field(0, description="Chunks processed in current ingestion run")
    total_chunks: int = Field(0, description="Total chunks expected for current ingestion run")
    progress: int = Field(0, description="Approximate ingestion progress percentage")
