...
def _project_root() -> Path:
    ...
    # utils/ocr/server.py -> utils -> repo root
    try:
...
def _http_json(method: str, url: str, payload: dict | None, headers: dict[str, str]) -> dict:
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail="Invalid MinerU response") from exc
...
def _http_put(url: str, file_path: Path) -> None:
    ...
    if not parts.scheme or not parts.netloc:
        raise HTTPException(status_code=502, detail="Invalid upload URL from MinerU")
    ...
    try:
        ...
        if resp.status >= 400:
            ...
            raise HTTPException(status_code=resp.status, detail=detail or f"Upload failed: {resp.status}")
...
def _download_and_extract_markdown(zip_url: str) -> tuple[str, str, list[tuple[str, bytes]]]:
    try:
        ...
        raise HTTPException(status_code=502, detail=f"Download result failed: {exc}") from exc

    with zipfile.ZipFile(BytesIO(data)) as zf:
        md_names = [n for n in zf.namelist() if n.lower().endswith(".md")]
        if not md_names:
            raise HTTPException(status_code=502, detail="No markdown found in MinerU output")
        ...
        try:
            ...
            if image_items:
                ...
                for rel_name, blob in image_items:
                    ...
                    safe_path = images_dir / rel_path.name if "images" not in rel_path.parts else images_dir / Path(*rel_path.parts[rel_path.parts.index("images") + 1 :])
                    try:
            ...
            if isinstance(exc, HTTPException):
                msg = _compact_upload_error(str(exc.detail))
            elif exc_msg:
                msg = exc_msg
            else:
                msg = ""
...
def list_sources() -> SourcesResponse:
    ...
    sources.append(SourceItem(key="(root)", label="(根目录)", total_files=total_files))

    for d in sorted([p for p in root.iterdir() if p.is_dir()], key=lambda p: p.name.lower()):
        cnt = len(_list_source_files(d, recursive=True))
        sources.append(SourceItem(key=d.name, label=d.name, total_files=cnt))

    return SourcesResponse(root=str(root), sources=sources)
...
def list_destinations() -> DestinationsResponse:
    ...
    return DestinationsResponse(root=str(root), destinations=destinations_sorted)
...
def create_job_from_source(request: CreateJobFromSourceRequest) -> dict:
    ...
    source_key = (request.source or "").strip()
    ...
    # destination is an optional folder name under OCR_OUTPUT_DIR; empty means root.
    destination = (request.destination or "").strip()
    if destination:
        dest_path = _safe_subdir(out_root, destination)
        dest_path.mkdir(parents=True, exist_ok=True)

    files = _list_source_files(source_dir, recursive=request.recursive)
    if not files:
        raise HTTPException(status_code=400, detail="No supported files in source directory")
    ...
    for p in files:
        tmp_file = tmp_dir / f"{uuid.uuid4().hex}_{p.name}"
        tmp_file.write_bytes(p.read_bytes())
        file_id = uuid.uuid4().hex
        file_entries.append(
            {
                "id": file_id,
    ...
    return {"job_id": job_id, "files": [{"id": f["id"], "file_name": f["file_name"]} for f in file_entries]}
...
def ocr_summary() -> OCRSummary:
    ...
    if output_root.exists():
        for md in output_root.rglob("*.md"):
            ...
            cat = categories.setdefault(category, {"total_files": 0, "total_pages": 0})
            cat["total_files"] += 1
            ...
            documents.append(
                {
                    ...
                    "pages": pages,
                    "updated_at": meta.get("updated_at") or time.strftime(
                        "%Y-%m-%d %H:%M:%S", time.localtime(md.stat().st_mtime)
                    ),
                }
            )
