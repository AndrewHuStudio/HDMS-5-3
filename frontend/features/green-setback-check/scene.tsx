...
export function GreenSetbackSceneLayer() {
  ...
  const selectedAreaId = useGreenSetbackStore((state) => state.selectedAreaId);
  const setSelectedAreaId = useGreenSetbackStore((state) => state.setSelectedAreaId);
  const meshList = sceneSnapshot?.meshList ?? [];
  const originalMaterials = useRef<Map<string, THREE.Material | THREE.Material[]>>(new Map());
  const greenAreas = result?.green_areas ?? [];
  ...
  const areaMetaMap = useMemo(() => {
    ...
    areaResults.forEach((item, index) => {
      const areaName = normalizeText(item.name) ?? `绿地${index + 1}`;
      const plotName =
        ...
        null;
      map.set(
        buildSetbackAreaSelectionId("green", index, item.id, item.name),
  ...
  const greenAreaMaterial = useMemo(
    () =>
  ...
  useEffect(() => {
    return () => {
      greenAreaMaterial.dispose();
    };
  }, [greenAreaMaterial]);

  const selectedGreenAreaMaterial = useMemo(
    () =>
      new THREE.MeshBasicMaterial({
        color: 0x16a34a,
        transparent: true,
  ...
  useEffect(() => {
    return () => {
      selectedGreenAreaMaterial.dispose();
    };
  }, [selectedGreenAreaMaterial]);

  const greenAreaMeshes = useMemo(() => {
    if (!greenAreas.length || extrudeHeight <= 0) return [];
    return greenAreas
      .map((area, index) => {
        ...
        const id = buildSetbackAreaSelectionId("green", index, area.id, area.name);
        return {
          key: `green-area-${index}`,
          id,
          areaName: normalizeText(area.name) ?? `绿地${index + 1}`,
          plotName:
            normalizeText(area.plot_name) ??
            areaShapePlotNameMap.get(normalizeText(area.name) ?? `绿地${index + 1}`) ??
            areaPlotNameMap.get(normalizeText(area.name) ?? `绿地${index + 1}`) ??
            null,
          shape,
          baseZ,
  ...
  }, [greenAreas, extrudeHeight, areaPlotNameMap, areaShapePlotNameMap]);

  const areaLabels = useMemo(() => {
    if (!greenAreaMeshes.length) return [];
    return greenAreaMeshes.map((mesh) => {
    ...
    <>
      {greenAreaMeshes.map((mesh) => (
        <mesh key={mesh.key} position={[0, 0, mesh.baseZ]}>
          ...
          />
          <primitive
            object={selectedAreaId === mesh.id ? selectedGreenAreaMaterial : greenAreaMaterial}
            attach="material"
          />
        </mesh>
      ))}
  ...
  return (
    <>
      {showHighlights && greenAreaMeshes.length > 0 && (
        modelTransform ? (
          <group
