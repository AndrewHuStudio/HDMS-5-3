...
- ~~热门问题缓存（Redis），避免重复检索和生成~~ **已实现**（内存 LRU+TTL 缓存，默认 128 条 / 1 小时过期）
- ~~Embedding 缓存，相同查询不重复调用嵌入 API~~ **已实现**（内存 LRU 缓存，默认 256 条）
- 数据库连接池优化
...
- ~~来源引用卡片优化：展示更多上下文、高亮匹配段落~~ **已实现**（摘要预览 + 关键词高亮 + 元数据标签）
- 知识图谱可视化联动：点击答案中的地块编号，图谱自动定位
- ~~对话导出：支持将问答记录导出为 PDF 或 Markdown~~ **已实现**（Markdown 导出）

---

## 第二轮优化：RAG 体验与性能（2026-02-08）

### 后端性能优化

| 优化项 | 说明 | 配置 |
|--------|------|------|
| **Embedding 缓存** | `embed_text()` 加内存 LRU 缓存，相同查询不重复调 API | `EMBEDDING_CACHE_MAX_SIZE`（默认 256） |
| **热门问答缓存** | 完整答案 + 来源缓存，命中时跳过检索和 LLM 调用 | `QUERY_CACHE_ENABLED`、`QUERY_CACHE_MAX_SIZE`（128）、`QUERY_CACHE_TTL_SECONDS`（3600） |
| **分数归一化** | 三路检索结果 min-max 归一化到 [0,1]，消除不同来源分数尺度差异 | - |
| **动态权重** | 根据查询内容自动调整融合权重：地块编号 → 图谱优先；指标关键词 → 图谱适度提升；默认 → 向量优先 | - |
| **Schema 统一** | 路由层使用 `schemas/rag_schemas.py` 中的模型，暴露 `top_k` 和 `use_retrieval` 参数 | - |

### 新增 API 端点

| 端点 | 说明 |
|------|------|
| `POST /qa/feedback` | 提交答案质量反馈（有用/没用），存入 MongoDB `qa_feedback` 集合 |
| `GET /qa/cache/stats` | 查看查询缓存统计（命中率、大小等） |

### 新增 SSE 事件

| 事件 | 说明 |
|------|------|
| `retrieval_stats` | 在 `sources` 之后、`thinking` 之前发送，包含各来源命中数、融合权重、是否缓存命中 |

### 前端体验优化

| 功能 | 说明 | 文件 |
|------|------|------|
| **检索过程可视化** | 可折叠面板，显示向量/图谱/关键词各命中数、权重百分比、缓存命中标签 | `components/qa-retrieval-stats.tsx` |
| **来源卡片增强** | 折叠态显示摘要（2 行）；展开显示关键词高亮全文 + 元数据标签（分类/含表格/含图片） | `components/qa-sources.tsx` |
| **答案反馈** | ThumbsUp/ThumbsDown 按钮，乐观更新 UI，后端存储反馈数据 | `components/qa-feedback.tsx` |
| **对话导出** | Header 右侧导出按钮，生成 Markdown 文件（含 Q&A + 来源引用） | `components/qa-export-button.tsx`、`lib/export-conversation.ts` |
